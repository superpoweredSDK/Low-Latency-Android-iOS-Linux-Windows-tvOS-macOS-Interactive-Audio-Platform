...
typedef struct inputOrOutputHandlerInternals {
    ...
    DWORD workQueueId;
    int bufferSize, numberOfFrames, samplerate, numberOfChannels;
...
public:
        ...
        if (internals->input) { // Handle audio input.
                ...
                if (SUCCEEDED(internals->captureClient->GetBuffer(&buffer, &framesAvailable, &captureFlags, &devicePosition, &qpcPosition))) {
                    ...
                    if (internals->hasOtherHandler) {
                        ...
                        if (internals->numberOfChannels != NUMCHANNELS) {
                            ...
                            internals->callback(internals->clientdata, internals->channelConvBuffer, internals->numberOfFrames, internals->samplerate);
                        } else internals->callback(internals->clientdata, (float *)buffer, internals->numberOfFrames, internals->samplerate);
                    }
            ...
            if (framesLeft > 0) {
                ...
                if (internals->channelConvBuffer) {
                    while (framesLeft >= internals->numberOfFrames) {
                        if (internals->hasOtherHandler) (*(internals->ioHandler))->handleOutput((BYTE *)internals->channelConvBuffer, internals->numberOfFrames);
                        
                        if (!internals->callback(internals->clientdata, internals->channelConvBuffer, internals->numberOfFrames, internals->samplerate)) memset(internals->channelConvBuffer, 0, internals->numberOfFrames * sizeof(float) * NUMCHANNELS); else silence = false;
                        
                        channelConversion(internals->channelConvBuffer, (float *)buffer, NUMCHANNELS, internals->numberOfChannels, internals->numberOfFrames);
                        
                        framesLeft -= internals->numberOfFrames;
                        framesWritten += internals->numberOfFrames;
                        buffer += internals->numberOfFrames * sizeof(float) * internals->numberOfChannels;
                    }
                } else {
                    while (framesLeft >= internals->numberOfFrames) {
                        if (internals->hasOtherHandler) (*(internals->ioHandler))->handleOutput(buffer, internals->numberOfFrames);
                        
                        if (!internals->callback(internals->clientdata, (float *)buffer, internals->numberOfFrames, internals->samplerate)) memset(buffer, 0, internals->numberOfFrames * sizeof(float) * internals->numberOfChannels); else silence = false;
	...
	STDMETHOD(ActivateCompleted)(IActivateAudioInterfaceAsyncOperation *operation) {
        ...
        internals->numberOfFrames = minPeriodFrames;
        ...
        if (!internals->hasOtherHandler || !internals->input) internals->callback(internals->clientdata, NULL, internals->samplerate, internals->samplerate);
...
private:
    ...
    HRESULT fail(int code) {
        ...
        internals->callback(internals->clientdata, NULL, -1, code);
...
void SuperpoweredWindowsAudioIO::start() {
    ...
    if (FAILED(MFStartup(MF_VERSION, MFSTARTUP_LITE))) {
        internals->callback(internals->clientdata, NULL, -1, 1);
        return;
    }
    ...
    if (FAILED(MFLockSharedWorkQueue(L"Pro Audio", 0, &taskId, &internals->workQueueId))) {
        MFShutdown();
        internals->callback(internals->clientdata, NULL, -1, 2);
        return;
    }
